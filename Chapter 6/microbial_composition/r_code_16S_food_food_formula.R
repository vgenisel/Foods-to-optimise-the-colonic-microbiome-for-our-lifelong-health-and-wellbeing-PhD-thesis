...
ps_food_food_formula <- ps_fixed %>%
  ps_filter(Category %in% c("food_and_food_and_formula"), .keep_all_taxa = TRUE)

#Alpha-diversity 
alpha_div <- estimate_richness(ps_food_food_formula, measures = c("Shannon", "Simpson", "Chao1"))

alpha_food <- plot_richness(ps_food_food_formula, x = "Description", color="Description", measures = c("Shannon", "Simpson", "Chao1")) +
...
#Rarefaction or not?
sample_sums(ps_food_food_formula) #to check number of reads per sample
set.seed(111) # keep result reproductive
ps_rarefied = rarefy_even_depth(ps_food_food_formula, sample.size = 51786, rngseed = 123) #rarefying to the minimal value of reads
...
ps_transformed <- ps_food_food_formula %>%
...
write.csv(agg_stats, "relative_abundance_with_variation_food_food_formula_genus.csv")
...
tse = mia::makeTreeSummarizedExperimentFromPhyloseq(ps_food_food_formula)
tse$Description <- factor(tse$Description)
tse$Description = relevel(tse$Description, ref = "Black_beans_and_Blackcurrant_and_formula")
...
write.csv(res_global, file = "ancombc2_global_food_food_formula_genus.csv", row.names = FALSE)

#multiple pairwise comparisons against a pre-specified group (Dunnett’s type of test)
res_dunn = output$res_dunn
write.csv(res_dunn, file = "ancombc2_dunn_blackbeans_blackcurrant_formula_phylum.csv", row.names = FALSE)
