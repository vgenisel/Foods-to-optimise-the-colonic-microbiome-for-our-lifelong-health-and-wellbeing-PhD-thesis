...
##Testing normallity for food_formula samples
df = pd.read_csv('data_z_scores.csv')
df_filtered = df[df['Treatment'].isin(['Food_formula'])]
analyte = 'total_SCFAs'
...
##Bland-Altman analysis for food_formula samples
...
df_filtered = df[df["Treatment"].isin(["Food_formula"])]
# Define analytes
analytes = ["Acetate", "Propionate", "total_SCFAs"]
...
fig.suptitle(f"Bland-Altman analysis for food_formula samples", fontsize=22, fontweight='bold')
